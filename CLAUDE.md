...
- **Twin file for Deno:** the Supabase Edge Function lives in a separate Deno runtime that cannot import from `/lib`. Therefore `supabase/functions/_shared/gemini.ts` exists as a **byte-identical** twin of `lib/gemini.ts`. Any change to one must be made in the other.
